        ...
        <div style="padding-top: 30px; background: white;padding-left: 20px;padding-right: 20px;">
            ...
            <div class="patient-result" style="margin: auto; width: 60%">
                ...
                <div class="body-result" style="padding-bottom: 20px;">
                    <form method="post" action="/admin/patient/{{$patient->id}}/result/{{$result->id}}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <div class="form-group {{ $errors->has('schedule_id') ? ' has-error' : '' }}">
                            ...
                            <div class="select-schedule row">
                                <input type="hidden" name="schedule_id" value="{{$result->schedule_id}}"/>
                                <div class="col-sm-10">
                                    <span class="form-control boxed" style="height: 35px;">
                                        {{ $result->schedule->title }} |
                                        {{ date('d/m/Y',strtotime($result->schedule->start_date)) }} -
                                        {{ date('d/m/Y',strtotime($result->schedule->end_date)) }}
                                    </span>
                                </div>
                        ...
                        <div class="form-group {{ $errors->has('vaccine_id') ? ' has-error' : '' }}">
                            ...
                            <select class="form-control boxed height-select" name="vaccine_id">
                                ...
                                @foreach($vaccines as $vaccine)
                                    <option value="{{$vaccine->id}}">{{ $vaccine->name }} - {{ $vaccine->category->name }}</option>
                                @endforeach
                            </select>
